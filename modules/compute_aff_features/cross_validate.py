...
def cross_validate(features, labels):
	...
	for count in range(n):
		...
		training_labels = labels[int(math.floor(num_features/10)):]
		testing_features = features[:int(math.floor(num_features/10))]
		testing_labels = labels[:int(math.floor(num_features/10))]
		
		data_x, data_y = training_features, training_labels
		# model = SVC()
		model = RandomForestClassifier()
		model.fit(data_x, data_y)

		data_x, data_y = testing_features, testing_labels

		pred_y = model.predict(data_x)
		error = pred_y - data_y
		e = np.count_nonzero(error)*1.0/len(data_y)
		total_error += e * 100
